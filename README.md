...
| version | Link |
| --- | --- |
| Scala 2.11 | `gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-askdre-0.0.20.jar` ([HTTP link](https://storage.googleapis.com/spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-askdre-0.0.20.jar)) |
| Scala 2.12 | `gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-askdre-0.0.20.jar` ([HTTP link](https://storage.googleapis.com/spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-askdre-0.0.20.jar)) |
| Spark 2.4  | `gs://spark-lib/bigquery/spark-2.4-bigquery-askdre-0.0.20-preview.jar`([HTTP link](https://storage.googleapis.com/spark-lib/bigquery/spark-2.4-bigquery-askdre-0.0.20-preview.jar)) |
...
| --- | --- |
| Scala 2.11 | `com.google.cloud.spark:spark-bigquery-with-dependencies_2.11:askdre-0.0.20` |
| Scala 2.12 | `com.google.cloud.spark:spark-bigquery-with-dependencies_2.12:askdre-0.0.20` |
| Spark 2.4  | `com.google.cloud.spark:spark-2.4-bigquery:askdre-0.0.20-preview` |
...
gcloud dataproc jobs submit pyspark --cluster "$MY_CLUSTER" \
  --jars gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-askdre-0.0.20.jar \
...
```
gcloud dataproc jobs submit pyspark --cluster "$MY_CLUSTER" \
  --jars gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-askdre-0.0.20.jar \
  examples/python/shakespeare.py
```
...
**Important:** Please use version askdre-0.0.20 and above for direct writes, as previous
...
pyspark --jars gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-askdre-0.0.20.jar
  --py-files gs://spark-lib/bigquery/spark-bigquery-support-askdre-0.0.20.zip
  --files gs://spark-lib/bigquery/spark-bigquery-support-askdre-0.0.20.zip
```
...
spark = SparkSession.builder\
  .appName('BigNumeric')\
  .config('spark.jars', 'gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-askdre-0.0.20.jar')\
  .config('spark.submit.pyFiles', 'gs://spark-lib/bigquery/spark-bigquery-support-askdre-0.0.20.zip')\
  .config('spark.files', 'gs://spark-lib/bigquery/spark-bigquery-support-askdre-0.0.20.zip')\
  .getOrCreate()

# extract the spark-bigquery-support zip file
import zipfile
with zipfile.ZipFile(SparkFiles.get("spark-bigquery-support-askdre-0.0.20.zip")) as zf:
  zf.extractall()
```
...
spark.sparkContext.addPyFile("gs://spark-lib/bigquery/spark-bigquery-support-askdre-0.0.20.zip")
```
...
from pyspark.sql import SparkSession
spark = SparkSession.builder
  .config("spark.jars.packages", "com.google.cloud.spark:spark-bigquery-with-dependencies_2.11:askdre-0.0.20")
  .getOrCreate()
...
com.google.cloud.spark:spark-bigquery-with-dependencies_**2.12**:askdre-0.0.20. In
...
```sbt
libraryDependencies += "com.google.cloud.spark" %% "spark-bigquery-with-dependencies" % "askdre-0.0.20"
```
