...
| version | Link |
| --- | --- |
| Scala 2.11 | `gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-0.0.25.jar` ([HTTP link](https://storage.googleapis.com/spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-0.0.25.jar)) |
| Scala 2.12 | `gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-0.0.25.jar` ([HTTP link](https://storage.googleapis.com/spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-0.0.25.jar)) |
| Spark 2.4  | `gs://spark-lib/bigquery/spark-2.4-bigquery-0.0.25-preview.jar`([HTTP link](https://storage.googleapis.com/spark-lib/bigquery/spark-2.4-bigquery-0.0.25-preview.jar)) |
...
| --- | --- |
| Scala 2.11 | `com.google.cloud.spark:spark-bigquery-with-dependencies_2.11:0.0.25` |
| Scala 2.12 | `com.google.cloud.spark:spark-bigquery-with-dependencies_2.12:0.0.25` |
| Spark 2.4  | `com.google.cloud.spark:spark-2.4-bigquery:0.0.25-preview` |
...
gcloud dataproc jobs submit pyspark --cluster "$MY_CLUSTER" \
  --jars gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-0.0.25.jar \
...
```
gcloud dataproc jobs submit pyspark --cluster "$MY_CLUSTER" \
  --jars gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-0.0.25.jar \
  examples/python/shakespeare.py
```
...
**Important:** Please use version 0.0.25 and above for direct writes, as previous
...
pyspark --jars gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-0.0.25.jar
  --py-files gs://spark-lib/bigquery/spark-bigquery-support-0.0.25.zip
  --files gs://spark-lib/bigquery/spark-bigquery-support-0.0.25.zip
```
...
spark = SparkSession.builder\
  .appName('BigNumeric')\
  .config('spark.jars', 'gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.11-0.0.25.jar')\
  .config('spark.submit.pyFiles', 'gs://spark-lib/bigquery/spark-bigquery-support-0.0.25.zip')\
  .config('spark.files', 'gs://spark-lib/bigquery/spark-bigquery-support-0.0.25.zip')\
  .getOrCreate()

# extract the spark-bigquery-support zip file
import zipfile
with zipfile.ZipFile(SparkFiles.get("spark-bigquery-support-0.0.25.zip")) as zf:
  zf.extractall()
```
...
spark.sparkContext.addPyFile("gs://spark-lib/bigquery/spark-bigquery-support-0.0.25.zip")
```
...
from pyspark.sql import SparkSession
spark = SparkSession.builder
  .config("spark.jars.packages", "com.google.cloud.spark:spark-bigquery-with-dependencies_2.11:0.0.25")
  .getOrCreate()
...
com.google.cloud.spark:spark-bigquery-with-dependencies_**2.12**:0.0.25. In
...
```sbt
libraryDependencies += "com.google.cloud.spark" %% "spark-bigquery-with-dependencies" % "0.0.25"
```
